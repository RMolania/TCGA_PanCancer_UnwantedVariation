...
#=================== RLE =================
## expr.data: gene expression data genes by samples
## is.log:  logical factor indicating whether the data is log transformed or not
...
#=================== Fast RUV-III normalization  =================
# Removing Unwanted Variation - III (RUV-III)
## Y: raw gene expression matrix (samples by genes) without PRPS 
## Yrep: raw gene expression matrix (samples by genes) of PRPS sets
## M: replicate matrix
## ctl: negative control genes
## k: the number of unwanted factors to use. 
...
dataSets.colors.3 <- c(dataSets.colors, 'cyan')
names(dataSets.colors.3)[5] <- 'RUV-III-P'
